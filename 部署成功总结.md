# 🎉 部署成功总结

## ✅ 部署状态

**应用已成功部署并运行！**

- ✅ Node.js 20.19.5 已安装
- ✅ PM2 进程管理器已安装并运行
- ✅ Docker 和 Docker Compose 已安装
- ✅ PostgreSQL 数据库已启动
- ✅ 项目依赖已安装
- ✅ Prisma Client 已生成
- ✅ 数据库迁移已完成
- ✅ Next.js 项目已构建
- ✅ PM2 服务已启动（状态：online）
- ✅ 开机自启已配置

## 🌐 访问应用

### 本地访问
```bash
curl http://localhost:3000
```

### 外部访问
- **IP 地址**: http://38.175.195.104:3000
- **服务器**: 38.175.195.104
- **端口**: 3000

## 🔧 服务管理命令

### 查看服务状态
```bash
pm2 status
pm2 list
```

### 查看日志
```bash
# 查看所有日志
pm2 logs maclock

# 查看最近 50 行日志
pm2 logs maclock --lines 50

# 实时查看日志
pm2 logs maclock --follow
```

### 服务控制
```bash
# 重启服务
pm2 restart maclock

# 停止服务
pm2 stop maclock

# 启动服务
pm2 start maclock

# 删除服务
pm2 delete maclock
```

### 监控服务
```bash
# 实时监控
pm2 monit

# 查看详细信息
pm2 show maclock
```

## 🔒 重要配置

### 1. 防火墙配置

确保端口 3000 已开放：

```bash
# 检查防火墙状态
ufw status

# 开放端口 3000
ufw allow 3000/tcp

# 如果需要 HTTP/HTTPS
ufw allow 80/tcp
ufw allow 443/tcp

# 重启防火墙
ufw reload
```

### 2. 环境变量配置

**重要**: 需要更新 `.env.local` 文件中的 API 密钥：

```bash
cd /var/www/maclock
nano .env.local
```

需要替换的占位符：
- `STRIPE_SECRET_KEY` → 您的 Stripe 密钥
- `NEXT_PUBLIC_STRIPE_PRICE_ID` → 您的 Stripe Price ID
- `CLERK_PUBLISHABLE_KEY` → 您的 Clerk 公钥
- `CLERK_SECRET_KEY` → 您的 Clerk 密钥
- `RESEND_API_KEY` → 您的 Resend API 密钥（如果使用）

更新后重启服务：
```bash
pm2 restart maclock
```

### 3. Nginx 反向代理（可选）

如果需要使用域名访问，配置 Nginx：

```bash
# 安装 Nginx
apt-get install -y nginx

# 创建配置文件
nano /etc/nginx/sites-available/maclock
```

配置文件内容（参考 `nginx.conf.example`）：
```nginx
server {
    listen 80;
    server_name your-domain.com;  # 替换为您的域名

    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

启用站点：
```bash
ln -s /etc/nginx/sites-available/maclock /etc/nginx/sites-enabled/
nginx -t
systemctl restart nginx
```

## 📊 数据库管理

### 查看数据库状态
```bash
docker-compose ps
```

### 连接数据库
```bash
docker-compose exec postgres psql -U maclock -d maclock
```

### 备份数据库
```bash
docker-compose exec postgres pg_dump -U maclock maclock > backup_$(date +%Y%m%d).sql
```

### 恢复数据库
```bash
docker-compose exec -T postgres psql -U maclock maclock < backup_20241121.sql
```

## 🔄 更新代码

当需要更新代码时：

```bash
cd /var/www/maclock

# 1. 上传新文件（使用 WinSCP 或 git）

# 2. 安装新依赖（如果有）
npm install

# 3. 运行数据库迁移（如果有）
npx prisma migrate deploy

# 4. 重新构建
npm run build

# 5. 重启服务
pm2 restart maclock
```

## 🐛 故障排除

### 应用无法访问

1. **检查服务状态**
   ```bash
   pm2 status
   pm2 logs maclock
   ```

2. **检查端口是否监听**
   ```bash
   netstat -tulpn | grep 3000
   ```

3. **检查防火墙**
   ```bash
   ufw status
   ```

### 数据库连接失败

1. **检查数据库容器**
   ```bash
   docker-compose ps
   docker-compose logs postgres
   ```

2. **检查环境变量**
   ```bash
   cat .env.local | grep DATABASE_URL
   ```

### 构建失败

1. **清理缓存**
   ```bash
   rm -rf node_modules .next
   npm install
   npm run build
   ```

2. **检查 Node.js 版本**
   ```bash
   node -v  # 应该是 18+ 或 20+
   ```

## 📝 常用检查清单

- [ ] 防火墙已开放端口 3000
- [ ] 环境变量已正确配置（.env.local）
- [ ] 数据库正在运行
- [ ] PM2 服务状态为 online
- [ ] 应用可以正常访问
- [ ] 日志中没有错误信息

## 🎯 下一步

1. **测试应用功能**
   - 访问首页
   - 测试登录功能
   - 测试支付流程（如果已配置 Stripe）

2. **配置域名**（可选）
   - 设置 DNS 记录
   - 配置 Nginx 反向代理
   - 配置 SSL 证书（Let's Encrypt）

3. **监控和维护**
   - 定期检查日志
   - 监控服务器资源使用
   - 定期备份数据库

## 📞 需要帮助？

如果遇到问题，请检查：
- PM2 日志：`pm2 logs maclock`
- 系统日志：`journalctl -u pm2-root`
- 数据库日志：`docker-compose logs postgres`

---

**恭喜！您的 Maclock 项目已成功部署！** 🎉






